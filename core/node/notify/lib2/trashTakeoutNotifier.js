...
class TrashTakeoutNotifier extends ScopedNotifier {
  ...
  check() {
    ...
    const { strTomorrow, strIn, capitalizeFirstLetter } = localize(program);
    ...
    for (const daysBefore of notifyDaysBefore) {
      ...
      if (times.find(t => isSameMinute(date, parseTimeToday(t)))) {
        ...
        if (list.length > 0) {
          ...
          const tagline = `${isDayBefore ? TOMORROW_SYMBOL : CALENDAR_SYMBOL} ${isDayBefore ? strTomorrow : inDays}`.trim();
